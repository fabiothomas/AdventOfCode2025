...
const rotate = (dir: string, state: number, add: number) => 
    add * (dir === 'L' ? -1 : 1) + state;

// read rotation from specific notation and execute
const execute = (step: string, state: number) => 
    rotate(step[0], state, parseInt(step.slice(1)));

// modulo with tracking of iterations
const modWithTracking = (n: number, i = 0): ModResult => 
    0 <= n && n < 100 ? { value: n, iterations: i } : 
    modWithTracking(n < 0 ? n + 100 : n - 100, i + 1);

// get correction for zeros passed based on specific edge cases
const getCorrection = (oldState: number, newState: number) => 
    oldState === 0 && newState < 0 ? -1 :
    newState > 0 && newState % 100 === 0 ? -1 :
    0;
